...
def get_loans():
    ...
    for loan in loans:
        ...
        loan_info = loan.as_dict()
        loan_info['book_title'] = book.title if book else 'Unknown'
        loan_info['customer_name'] = customer.name if customer else 'Unknown'
...
@app.route('/upload_image/<int:book_id>', methods=['POST', 'OPTIONS'])
@jwt_required()
def handle_options(book_id):
...
def upload_image(book_id):
    if request.method == 'OPTIONS':
        return '', 204  # Handle the preflight request
